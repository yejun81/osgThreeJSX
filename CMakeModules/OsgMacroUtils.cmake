...
MACRO(SETUP_LIBRARY LIB_NAME)
    ...
    ENDIF()
        ...
        ADD_LIBRARY(${LIB_NAME}
            ${OSGTHREEJSX_USER_DEFINED_DYNAMIC_OR_STATIC}
            ${TARGET_H}
...
MACRO(SETUP_PLUGIN PLUGIN_NAME)
    ...
    ## plugins gets put in libosgthreejsx by default
    IF(${ARGC} GREATER 1)
      SET(PACKAGE_COMPONENT libosgthreejsx-${ARGV1})
      ...
      ENDIF()
    ELSE(${ARGC} GREATER 1)
      SET(PACKAGE_COMPONENT libosgthreejsx)
    ENDIF(${ARGC} GREATER 1)

    # Add the VisualStudio versioning info
    SET(TARGET_SRC ${TARGET_SRC} ${OSGTHREEJSX_VERSIONINFO_RC})

    # here we use the command to generate the library

    IF   (DYNAMIC_OSGTHREEJSX)
        ADD_LIBRARY(${TARGET_TARGETNAME} MODULE ${TARGET_SRC} ${TARGET_H})
    ELSE (DYNAMIC_OSGTHREEJSX)
        ADD_LIBRARY(${TARGET_TARGETNAME} STATIC ${TARGET_SRC} ${TARGET_H})
    ENDIF(DYNAMIC_OSGTHREEJSX)
...
#the installation path are differentiated for win32 that install in bin versus other architecture that install in ${OSG_INSTALL_LIBDIR}/${OSG_PLUGINS}
    IF(WIN32)
        INSTALL(TARGETS ${TARGET_TARGETNAME}
            ...
            ARCHIVE DESTINATION lib/${OSG_PLUGINS} COMPONENT libosgthreejsx-dev
    ...
    ELSE(WIN32)
        INSTALL(TARGETS ${TARGET_TARGETNAME}
            RUNTIME DESTINATION bin COMPONENT ${PACKAGE_COMPONENT}
            ARCHIVE DESTINATION ${OSG_INSTALL_LIBDIR}/${OSG_PLUGINS} COMPONENT libosgthreejsx-dev
